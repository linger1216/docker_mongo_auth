...
auth="-u admin -p $MONGO_PASSWD"
js="if (!db.getUser('admin')) { db.createUser({ user: 'admin', pwd: '$MONGO_PASSWD', roles: [ {role:'root', db:'admin'},{role: 'readWriteAnyDatabase', db: 'admin' }]}) }"
